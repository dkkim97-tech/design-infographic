...
| 위 5종 외 디자인 이름/무드 (아래 갤러리 15종) | `ppt-design-gallery` |

## PPT 디자인 갤러리 (20종 빠른 선택)

디자인 **이름이나 무드 키워드**로 슬라이드를 요청하면 `ppt-design-gallery` 스킬이 해당 디자인 토큰을 적용한다. 슬라이드 만들 때 "이름만 부르면" 되는 선택 창구.

갤러리 수록 15종 트리거: **Editorial**(에디토리얼·매거진) · **Gridwork**(스위스 그리드) · **Aurora**(오로라·파스텔 글래스) · **Noir**(느와르·다크 럭셔리·골드) · **Terminal**(터미널·사이버펑크·네온) · **Bloom**(블룸·파스텔 소프트) · **Press**(프레스·뉴스페이퍼·신문) · **Atlas**(아틀라스·미드센추리) · **Blueprint**(블루프린트·설계도) · **Hanji**(한지·전통·단청) · **Datablock**(데이터블록·대시보드) · **Botanical**(보태니컬·오가닉) · **Neumorph**(뉴모피즘·소프트 UI) · **Glassmorph**(글래스모피즘·다크 글래스) · **Claymorph**(클레이모피즘·클레이). 나머지 5종은 각 전용 스킬(위 표) 사용.
